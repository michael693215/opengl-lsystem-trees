...
const char *ShaderInfo::ShaderVersion = "";
...
bool loadShader(unsigned int id, const char *file)
{
    ...
    // we're using one block for the shader version and one for the entire code file.
    int shaderBlockSizes[] = {
        int(strlen(ShaderInfo::ShaderVersion)),
        int(statbuf.st_size)
    };
    std::vector<GLchar> shader(shaderBlockSizes[1]);
    GLchar *shaderBlocks[] = {
        (GLchar*)(ShaderInfo::ShaderVersion),
        shader.data()
    };

    FILE *f = fopen(path.c_str(), "rb");
    assert(f);
    fread(shaderBlocks[1], 1, shaderBlockSizes[1], f);
    fclose(f);

    // compile as shader
    glShaderSource(id, sizeof(shaderBlocks)/sizeof(*shaderBlocks), shaderBlocks, shaderBlockSizes);
    glCompileShader(id);
